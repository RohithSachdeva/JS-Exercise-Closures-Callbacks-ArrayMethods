...
function getFullNames(runners) {
  ...
  runners.forEach(function(item){
    runnerArray.push(`${item.last_name}, ${item.first_name}`);
  });
return runnerArray;
}
...
function firstNamesAllCaps(runners) {
  let runnerArray = runners.map(function(runner){
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let runnerShirts = runners.filter(runner => runner.shirt_size === tShirtSize);
  return runnerShirts;
}
